...
runTestsOnModel() {
  ...
  do
    ...
    do
      ...
      docker run -d --rm -v `pwd`:/app -w /app --name alphartc_pyinfer_temp --cap-add=NET_ADMIN challenge-env peerconnection_serverless receiver_pyinfer.json
      sleep 1
      docker exec -d -w /app/tc_profiles alphartc_pyinfer_temp bash ./tc_policy.sh $PROFILE
      docker exec alphartc_pyinfer_temp peerconnection_serverless sender_pyinfer.json
